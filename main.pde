...
void setup() {
  ...
  frameRate(60);
  fullScreen();
  herorun1 = loadImage("herorun0.png");
  herorun2 = loadImage("herorun1.png");
  herojump = loadImage("herojump.png");
  herolean = loadImage("herolean0.png");
  herolean1 = loadImage("herolean1.png");

  spikesmall = loadImage("spikesmall.png");
  spikebig = loadImage("spikebig.png");
  spikesmallmany = loadImage("spikesmallmany.png");
  bat = loadImage("bat0.png");
...
void writeInfo() {
  ...
  if (showBestEachGen) { //if showing the best for each gen then write the applicable info
    ...
    textSize(20);
    int x = 580;
    text("Растояние до следующего препятствия", x, 18+44.44444);
    text("Высота препядствия", x, 18+2*44.44444);
    text("Ширина препядствия", x, 18+3*44.44444);
    text("Высота летучей мыши", x, 18+4*44.44444);
    text("Скорость", x, 18+5*44.44444);
    text("Позиция игрока (Y)", x, 18+6*44.44444);
    text("Расстояние между препятствиями", x, 18+7*44.44444);
    text("Смещение", x, 18+8*44.44444);

    textAlign(LEFT);
    text("Небольшой прыжок", 1220, 118);
    text("Большой прыжок", 1220, 218);
    text("Нагнуться", 1220, 318);
  } else { //evolving normally 
    text("Счёт: " + floor(pop.populationLife/3.0), 30, height - 30);
    //text(, width/2-180, height-30);
    textAlign(RIGHT);

    text("Поколение: " + (pop.gen +1), width -40, height-30);
    textSize(20);
    int x = 580;
    text("Растояние до следующего препятствия", x, 18+44.44444);
    text("Высота препядствия", x, 18+2*44.44444);
...
void moveObstacles() {
  ...
  for (int i = 0; i< bats.size(); i++) {
    bats.get(i).move(speed);
    if (bats.get(i).posX < -playerXpos) {
      bats.remove(i);
      i--;
    }
  }
...
void addObstacle() {
  ...
  int tempInt;
  if (lifespan > 1000 && random(1) < 0.15) { // 15% of the time add a bat
    tempInt = floor(random(3));
    bat temp = new bat(tempInt);//floor(random(3)));
    bats.add(temp);
  } else {
    tempInt = floor(random(3));
...
//-------------------------------------------------------------------------------------------------------------------------------------------
//resets all the obstacles after every has died
void resetObstacles() {
